...
void draw_confirmation_modal(const char *header_text, const char *body_text, const char *submit_button_text, void (*_onsubmit)(void), void (*_oncancel)(void), uint8_t modal_type)
{
    ...
    // Draw Submit Button
    Rectangle submit_button_rec = (Rectangle){confirm_modal_rec.x + confirm_modal_rec.width - 200, confirm_modal_rec.y + confirm_modal_rec.height - 65, MeasureText(submit_button_text, 20) + 30, 40};
    ...
    DrawText(submit_button_text, submit_button_rec.x + 15, submit_button_rec.y + 10, 20, selected_index == BUTTON_SUBMIT ? DARKGRAY : BLACK);
